# Tugas 4 Progjar C 2020

## Soal 1 - Merancang Protokol

### Server
#### Dokumentasi Server
##### Bagian 1
![alt text](capture/0_dokumentasi_server_1.png)
##### Bagian 2
![alt text](capture/0_dokumentasi_server_2.png)
#### Sedang Menajalankan Server
![alt text](capture/1_a_menjalankan_server.png)

### a. Meletakkan (Upload) file
#### [1] sedang menjalankan client upload
![alt text](capture/1_a_menjalankan_client.png)
#### [2] sebelum menjalankan client upload
![alt text](capture/1_a_sebelum_upload.png)
#### [3] sesudah menjalankan client upload
![alt text](capture/1_a_sesudah_upload.png)

### b. Mengambil (Download) file
#### [1] sedang menjalankan client download
![alt text](capture/1_b_menjalankan_client.png)
#### [2] sebelum menjalankan client download
![alt text](capture/1_b_sebelum_download.png)
#### [3] sesudah menjalankan client download
![alt text](capture/1_b_sesudah_download.png)
#### [4] penampakan hasil download
![alt text](capture/1_b_penampakan_download.png)

### c. Melihat List File
#### [1] sedang menjalankan client list
![alt text](capture/1_c_directory_list_json.png)

## Soal 2 - Membuat Dokumentasi Protokol
![alt text](capture/2_dokumentasi_protokol.png)

## Soal 3 - Gunakan Format JSON
![alt text](capture/1_c_directory_list_json.png)

## Soal 4 - Buatlah client untuk setiap operasi
### Client Upload
![alt text](capture/4_client_upload.png)
### Client Download
![alt text](capture/4_client_download.png)
### Client List
![alt text](capture/4_client_list.png)